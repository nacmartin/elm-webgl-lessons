...
fetchTextures =
  loadTextureWithFilter WebGL.Nearest "textures/glass.gif" `Task.andThen` \nearestTexture ->
  ...
  in
    div
      ...
      , div
        ...
        [ div []
          [ input [type' "checkbox", onClick UseBlending, checked blending] []
          , text " Use blending"
...
renderEntity : Drawable { position:Vec3, coord:Vec3, norm: Vec3 } -> Float -> Float -> Maybe Texture -> Vec3 -> Bool -> Bool -> Float -> Vec3 -> Vec3 -> Vec3 -> List Renderable
renderEntity mesh thetaX thetaY texture position lighting blending alpha directionalColour directional ambientColour =
  let funcs =
    case blending of
     True -> [Enable Blend, Disable DepthTest, BlendFunc (SrcAlpha, One)]
     False -> [Disable Blend, Enable DepthTest]
  in
    case texture of
      Nothing ->
       []
      ...
      Just tex ->
       [renderWithConfig funcs vertexShader fragmentShader mesh (uniformsCube thetaX thetaY tex position lighting alpha directionalColour directional ambientColour)]
