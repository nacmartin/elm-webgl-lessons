...
fetchTextures =
  loadTextureWithFilter WebGL.Linear "textures/earth.jpg" `Task.andThen` \colorMap ->
  loadTextureWithFilter WebGL.Linear "textures/earth-specular.gif" `Task.andThen` \specularMap ->
...
sphereFace latitude1 latitude2 longitude1 longitude2 radius =
  let
    ...
    topLeft     = { position = vec3 ((cos theta2) * (sin phi1) * radius) ((sin theta2) * radius) ((cos theta2) * (cos phi1) * radius), coord = vec3 (longitude1-0.5) (latitude2-0.5) 0, normal = (vec3 ((cos theta2) * (sin phi1)) ((sin theta2)) ((cos theta2) * (cos phi1))) }
    topRight    = { position = vec3 ((cos theta2) * (sin phi2) * radius) ((sin theta2) * radius) ((cos theta2) * (cos phi2) * radius), coord = vec3 (longitude2-0.5) (latitude2-0.5) 0, normal = (vec3 ((cos theta2) * (sin phi2)) ((sin theta2)) ((cos theta2) * (cos phi2))) }
    bottomLeft  = { position = vec3 ((cos theta1) * (sin phi1) * radius) ((sin theta1) * radius) ((cos theta1) * (cos phi1) * radius), coord = vec3 (longitude1-0.5) (latitude1-0.5) 0, normal = (vec3 ((cos theta1) * (sin phi1)) ((sin theta1)) ((cos theta1) * (cos phi1))) }
    bottomRight = { position = vec3 ((cos theta1) * (sin phi2) * radius) ((sin theta1) * radius) ((cos theta1) * (cos phi2) * radius), coord = vec3 (longitude2-0.5) (latitude1-0.5) 0, normal =  (vec3 ((cos theta1) * (sin phi2)) ((sin theta1)) ((cos theta1) * (cos phi2))) }
  in
...
uniforms colorMap specularMap theta position useLighting useSpecularMap useColorMap specularColor specular ambientColor diffuseColor =
  let
    worldSpace = (translate position (makeRotate theta (vec3 0 1 0) ))
    camera = makeLookAt (vec3 0 0 3) (vec3 0 0 -1) (vec3 0 1 0)
